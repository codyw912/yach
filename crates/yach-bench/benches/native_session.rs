...
use yach_backend::{
    EntryId, JsonlSessionStore, Role, SessionEvent, SessionEventSink, SessionId, SessionLog,
    ToolOutcome, ToolPayloadSummary, ToolPermissionState, ToolRequestId, TurnId, TurnOutcome,
};
...
fn turn_events(index: usize) -> [SessionEvent; 5] {
    let session_id = SessionId(String::from("bench-session"));
    let turn_id = TurnId(format!("turn-{index}"));
    let user_entry_id = EntryId(format!("entry-{index}-user"));
    let assistant_entry_id = EntryId(format!("entry-{index}-assistant"));
    let tool_request_id = ToolRequestId(format!("tool-request-{index}"));
...
fn session_events(turns: usize) -> Vec<SessionEvent> {
...
fn write_session_file(turns: usize) -> Option<TempSessionFile> {
    ...
    let store = JsonlSessionStore::new(fixture.path().to_path_buf());
    store.append_events(&session_events(turns)).ok()?;
    Some(fixture)
}

fn fixture_log(turns: usize) -> SessionLog {
    SessionLog {
        events: session_events(turns),
    }
}

fn session_append_event(c: &mut Criterion) {
    let event = SessionEvent::TurnFinished {
        session_id: SessionId(String::from("bench-session")),
        turn_id: TurnId(String::from("turn-append")),
        outcome: TurnOutcome::Completed,
        reason: None,
    };

    c.bench_function("session_append_event", |b| {
        b.iter_batched(
...
fn session_load_10_turns(c: &mut Criterion) {
    bench_native_session_load(c, 10, "session_load_10_turns");
}

fn session_load_100_turns(c: &mut Criterion) {
    bench_native_session_load(c, 100, "session_load_100_turns");
}

fn session_load_1000_turns(c: &mut Criterion) {
    bench_native_session_load(c, 1000, "session_load_1000_turns");
}
...
fn session_projection_10_turns(c: &mut Criterion) {
    bench_native_session_projection(c, 10, "session_projection_10_turns");
}

fn session_projection_100_turns(c: &mut Criterion) {
    bench_native_session_projection(c, 100, "session_projection_100_turns");
}

fn session_projection_1000_turns(c: &mut Criterion) {
    bench_native_session_projection(c, 1000, "session_projection_1000_turns");
}
...
criterion_group!(
    ...
    session_load_10_turns,
    session_load_100_turns,
    session_load_1000_turns,
    session_projection_10_turns,
    session_projection_100_turns,
    session_projection_1000_turns,
);
