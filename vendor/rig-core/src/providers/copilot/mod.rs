                    ...
                    match event_result {
                        ...
                        Ok(crate::http_client::sse::Event::Message(evt)) => {
                            ...
                            let completed_output =
                                match responses_api::streaming::raw_completed_output(&evt.data) {
                                    Ok(output) => output,
                                    Err(error) => {
                            ...
                            let data = match serde_json::from_str::<
                                responses_api::streaming::StreamingCompletionChunk,
                            >(&evt.data) {
                                Ok(data) => data,
                                Err(error) => {
                            ...
                            if completed_output.is_some()
                                && !matches!(
                                    &data,
                                    responses_api::streaming::StreamingCompletionChunk::Response(chunk)
                                        if matches!(
                                            &chunk.kind,
                                ...
                                )
                            {
                                ...
                                yield Err(CompletionError::ResponseError(
                                    "response.completed did not decode as a terminal response"
                                        .to_string(),
                                ));
                                break;
                            }
                            ...
                            if let responses_api::streaming::StreamingCompletionChunk::Response(chunk) = data {
                                ...
                                match kind {
                                    responses_api::streaming::ResponseChunkKind::ResponseCompleted => {
                                        ...
                                        final_output = completed_output.unwrap_or_default();
                                    }
...
mod tests {
    ...
    #[tokio::test]
    async fn responses_stream_preserves_terminal_output_on_final_response() {
        let output = vec![
            serde_json::json!({
                "type": "message",
                "id": "msg_123",
    ...
    #[tokio::test]
    async fn responses_stream_rejects_completed_event_that_falls_back_to_delta() {
        let completed = serde_json::json!({
            "type": "response.completed",
            "output_index": 0,
            "sequence_number": 1,
        ...
        {
            Ok(_) => panic!("completed event must not fall back to a delta"),
            Err(error) => error,
        };
        assert!(matches!(
            error,
