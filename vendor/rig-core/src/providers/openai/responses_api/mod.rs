...
impl InputItem {
    ...
    /// Creates an input item that preserves a provider-native item verbatim.
    ///
    /// The value must be an object when serialized as an input item.
    pub fn unknown(value: Value) -> Self {
        Self {
            role: None,
            input: InputContent::Unknown(value),
        }
    }
}
...
#[derive(Serialize)]
#[serde(tag = "type", rename_all = "snake_case")]
enum KnownInputContentRef<'a> {
    Message(&'a Message),
    Reasoning(&'a OpenAIReasoning),
    FunctionCall(&'a OutputFunctionCall),
    FunctionCallOutput(&'a ToolResult),
}
...
impl Serialize for InputContent {
    ...
    {
        match self {
            Self::Message(message) => KnownInputContentRef::Message(message).serialize(serializer),
            Self::Reasoning(reasoning) => {
                KnownInputContentRef::Reasoning(reasoning).serialize(serializer)
            }
            Self::FunctionCall(function_call) => {
                KnownInputContentRef::FunctionCall(function_call).serialize(serializer)
            }
            Self::FunctionCallOutput(function_call_output) => {
                KnownInputContentRef::FunctionCallOutput(function_call_output).serialize(serializer)
            }
...
impl<'de> Deserialize<'de> for InputContent {
        ...
        D: Deserializer<'de>,
    {
        ...
        let type_name = value
            ...
            .ok_or_else(|| serde::de::Error::custom("Input content must be an object with a type"))?;

        match type_name {
            "message" | "reasoning" | "function_call" | "function_call_output" => {
                match serde_json::from_value(value).map_err(serde::de::Error::custom)? {
                    KnownInputContent::Message(message) => Ok(Self::Message(message)),
                    KnownInputContent::Reasoning(reasoning) => Ok(Self::Reasoning(reasoning)),
                    KnownInputContent::FunctionCall(function_call) => {
                        Ok(Self::FunctionCall(function_call))
                    }
                    KnownInputContent::FunctionCallOutput(function_call_output) => {
                        Ok(Self::FunctionCallOutput(function_call_output))
                    }
                }
            }
...
{
    /// Sends a caller-built OpenAI Responses request.
    pub async fn completion_with_request(
        ...
        if enabled!(Level::TRACE) {
            tracing::trace!(
                ...
                "OpenAI Responses completion request (content omitted)"
            );
        }
            ...
            if response.status().is_success() {
                ...
                if enabled!(Level::TRACE) {
                    tracing::trace!(
                        ...
                        "OpenAI Responses completion response (content omitted)"
                    );
                }
                response.try_into()
            } else {
    ...
    H: Clone + Default + std::fmt::Debug + WasmCompatSend + WasmCompatSync + 'static,
{
    ...
    async fn completion(
        ...
        self.completion_with_request(request, record_telemetry_content)
            .await
    }
    ...
    > {
        let record_telemetry_content = completion_request.record_telemetry_content;
        let request = self.create_completion_request(completion_request)?;
        self.stream_with_request(request, record_telemetry_content)
            .await
    }
}
...
mod tests {
    ...
    fn caller_built_request(raw_input: Value) -> CompletionRequest {
        CompletionRequest {
            input: OneOrMany::one(InputItem::unknown(raw_input)),
            ...
            temperature: None,
            tool_choice: None,
            tools: vec![ResponsesToolDefinition::function(
                "lookup",
                "Look up a value",
                json!({"type": "object"}),
            )],
    ...
    #[tokio::test]
    async fn completion_with_request_preserves_caller_built_request() {
        use crate::client::CompletionClient;
        use crate::providers::openai::Client;
        use crate::test_utils::RecordingHttpClient;
        ...
        let http_client = RecordingHttpClient::new(
            r#"{"id":"resp_123","object":"response","created_at":0,"status":"completed","model":"gpt-5.4"}"#,
        );
        let client = Client::builder()
            .api_key("test-key")
        ...
        let _ = model
            .completion_with_request(caller_built_request(raw_input.clone()), false)
            .await
            .expect_err("empty mock response should fail after recording the request");
    ...
    #[test]
    fn input_content_unknown_round_trips_unknown_type() {
        let raw = json!({
            "type": "web_search_call",
            "id": "ws_123",
            "status": "completed",
        ...
        let content: InputContent =
            serde_json::from_value(raw.clone()).expect("unknown input content should deserialize");

        assert!(matches!(&content, InputContent::Unknown(value) if value == &raw));
        assert_eq!(
            serde_json::to_value(content).expect("unknown input content should serialize"),
            raw
        );
    }

    #[test]
    fn input_item_unknown_preserves_caller_supplied_items() {
        for raw in [
            json!({
                "type": "message",
                "role": "assistant",
            ...
            assert_eq!(
                serde_json::to_value(InputItem::unknown(raw.clone()))
                    .expect("raw input item should serialize"),
                raw
            );
        }
    }
    ...
    #[test]
    fn input_item_unknown_rejects_non_object_raw_input() {
        assert!(
            serde_json::to_value(InputItem::unknown(json!("not an object"))).is_err(),
            "raw input must fail at the InputItem object boundary"
        );
        assert!(
            serde_json::from_value::<InputItem>(json!("not an object")).is_err(),
            "non-object input must not deserialize as an InputItem"
        );
    }
