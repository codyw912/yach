...
pub struct StreamingCompletionResponse {
    ...
    /// Raw output items from the terminal `response.completed` event, in provider order.
    #[serde(default)]
...
pub(crate) fn raw_completed_output(
    raw_event_data: &str,
) -> Result<Option<Vec<serde_json::Value>>, CompletionError> {
    let Ok(raw_event) = serde_json::from_str::<serde_json::Value>(raw_event_data) else {
        return Ok(None);
    };
    if raw_event.get("type").and_then(serde_json::Value::as_str) != Some("response.completed") {
        return Ok(None);
    }

    match raw_event.pointer("/response/output") {
        Some(serde_json::Value::Array(output)) => Ok(Some(output.clone())),
        Some(_) => Err(CompletionError::ResponseError(
            "response.completed output must be an array".to_string(),
        )),
        None => Err(CompletionError::ResponseError(
            "response.completed must include response.output".to_string(),
        )),
    }
}
...
impl RawChoiceAccumulator {
    ...
    fn record_response_chunk(
        ...
        match kind {
            ResponseChunkKind::ResponseCompleted => {
                self.raw_output = raw_completed_output(raw_event_data)?.unwrap_or_default();
        ...
        while let Some(event_result) = event_source.next().await {
            match event_result {
                ...
                Ok(Event::Message(evt)) => {
                    ...
                    let completed_output = match raw_completed_output(&evt.data) {
                        Ok(output) => output,
                        Err(error) => {
                            terminated_with_error = true;
                            yield Err(error);
                    ...
                    if completed_output.is_some()
                        && !matches!(
                            &data,
                            StreamingCompletionChunk::Response(chunk)
                                if matches!(&chunk.kind, ResponseChunkKind::ResponseCompleted)
                        )
                    {
                        terminated_with_error = true;
                        yield Err(CompletionError::ResponseError(
                            "response.completed did not decode as a terminal response".to_string(),
                        ));
                        break;
                    }
...
{
    /// Streams a caller-built OpenAI Responses request.
    pub async fn stream_with_request(
        &self,
        mut request: super::CompletionRequest,
        record_telemetry_content: bool,
    ) -> Result<streaming::StreamingCompletionResponse<StreamingCompletionResponse>, CompletionError>
    {
        let system_instructions = request.instructions.clone();
        ...
        if enabled!(Level::TRACE) {
            tracing::trace!(
                ...
                "OpenAI Responses streaming completion request (content omitted)"
            );
        }
...
mod tests {
    ...
    struct CapturingStreamingClient {
        requests: std::sync::Arc<std::sync::Mutex<Vec<bytes::Bytes>>>,
    }

    impl CapturingStreamingClient {
        fn request_bodies(&self) -> Vec<bytes::Bytes> {
    ...
    impl crate::http_client::HttpClientExt for CapturingStreamingClient {
        fn send<T, U>(
            &self,
            _req: crate::http_client::Request<T>,
        ) -> impl Future<
            Output = crate::http_client::Result<
                crate::http_client::Response<crate::http_client::LazyBody<U>>,
            >,
        ...
        fn send_multipart<U>(
            &self,
            _req: crate::http_client::Request<crate::http_client::MultipartForm>,
        ) -> impl Future<
            Output = crate::http_client::Result<
                crate::http_client::Response<crate::http_client::LazyBody<U>>,
            >,
        ...
        fn send_streaming<T>(
            &self,
            req: crate::http_client::Request<T>,
        ) -> impl Future<
            ...
            T: Into<bytes::Bytes> + crate::wasm_compat::WasmCompatSend,
        {
            ...
            match self.requests.lock() {
                Ok(mut requests) => requests.push(body),
                Err(poisoned) => poisoned.into_inner().push(body),
            }

            async {
                let events = futures::stream::iter(vec![Ok::<bytes::Bytes, crate::http_client::Error>(
                    bytes::Bytes::from_static(b"data: [DONE]\n\n"),
                )]);
                http::Response::builder()
                    .status(http::StatusCode::OK)
                    .header(http::header::CONTENT_TYPE, "text/event-stream")
                    .body(Box::pin(events) as crate::http_client::sse::BoxedStream)
                    .map_err(crate::http_client::Error::Protocol)
            }
        }
    }

    fn caller_built_stream_request(raw_input: serde_json::Value) -> super::super::CompletionRequest {
        super::super::CompletionRequest {
            input: crate::OneOrMany::one(super::super::InputItem::unknown(raw_input)),
            model: "gpt-5.4".to_string(),
            instructions: Some("Keep this instruction".to_string()),
            max_output_tokens: Some(128),
            stream: None,
            temperature: None,
            tool_choice: None,
            tools: vec![super::super::ResponsesToolDefinition::function(
                "lookup",
                "Look up a value",
                json!({"type": "object"}),
            )],
            additional_parameters: super::super::AdditionalParameters {
                store: Some(true),
                ..Default::default()
            },
        }
    ...
    #[tokio::test]
    async fn stream_with_request_preserves_caller_built_request() {
        let raw_input = json!({
            "type": "web_search_call",
            "id": "ws_123",
            "provider_field": {"preserved": true}
        });
        let http_client = CapturingStreamingClient::default();
        ...
        let mut stream = model
            .stream_with_request(caller_built_stream_request(raw_input.clone()), false)
            .await
            .expect("caller-built request should start");
        let _ = stream.next().await;

        let request_bodies = http_client.request_bodies();
        assert_eq!(request_bodies.len(), 1);
        let body: serde_json::Value =
            serde_json::from_slice(&request_bodies[0]).expect("request body should be JSON");
        assert_eq!(body["input"], json!([raw_input]));
    ...
    #[test]
    fn terminal_output_is_ordered_and_replayable() {
        let output = vec![
            json!({
                "type": "message",
                "id": "msg_123",
        ...
        let body = format!("data: {terminal_event}\n\n");

        let choices = raw_choices_from_sse_body(&body, ResponsesUsage::new())
            .expect("terminal event should decode");
        let final_response = choices
            .iter()
            .find_map(|choice| match choice {
                RawStreamingChoice::FinalResponse(response) => Some(response),
                _ => None,
            })
            .expect("terminal event should produce a final response");

        assert_eq!(final_response.output, output);
        let replayed_input = final_response
            .output
            .iter()
            .cloned()
            .map(super::super::InputItem::unknown)
            .collect::<Vec<_>>();
        assert_eq!(
            serde_json::to_value(replayed_input).expect("replayed input should serialize"),
            json!(output)
        );
    }

    #[tokio::test]
    async fn live_stream_rejects_completed_event_that_falls_back_to_delta() {
        let event = json!({
            "type": "response.completed",
            "output_index": 0,
            "sequence_number": 1,
        ...
        let error = first_error_from_event(event).await;
        assert!(matches!(
            error,
            crate::completion::CompletionError::ResponseError(message)
                if message == "response.completed did not decode as a terminal response"
        ));
    }
    #[tokio::test]
    async fn live_stream_rejects_malformed_terminal_output() {
        let event = json!({
            "type": "response.completed",
            "sequence_number": 1,
            "response": {
        ...
        assert!(matches!(
            error,
            crate::completion::CompletionError::ResponseError(message)
                if message == "response.completed output must be an array"
        ));
    }

    #[tokio::test]
    async fn live_stream_rejects_terminal_output_that_is_absent() {
        let event = json!({
            "type": "response.completed",
            "sequence_number": 1,
            "response": {
        ...
        assert!(matches!(
            error,
            crate::completion::CompletionError::ResponseError(message)
                if message == "response.completed must include response.output"
        ));
    }
