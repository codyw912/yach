...
- *(core)* Preserve one bounded `Retry-After` header on non-success HTTP
  responses before the body is consumed. The captured value is at most 128
  bytes, omitted from `Display`/`Debug`, and recoverable through
  `provider_response_retry_after()` on capability errors. The full header map
  is not retained.
