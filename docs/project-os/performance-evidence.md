...
| 2026-04-27 | Live Crossterm 10,000-entry and 50,000-entry transcript scroll-to-draw/flush paths | 10,000-entry 200-sample run: p50 3.688 ms, p95 4.435 ms, p99 4.966 ms, max 5.556 ms. 50,000-entry 50-sample stress run: p50 19.963 ms, p95 22.962 ms, p99/max 23.895 ms. | Narrow yach-only live-terminal evidence that 10,000-entry transcript scroll/redraw is below a 16 ms frame budget on this machine; 50,000-entry stress confirms scaling risk and exceeds the 16 ms frame budget. | Yach-only; synthetic fixture; excludes OS input/event polling; does not prove bounded visible-work rendering or Pi advantage. | `../benchmarks/transcript-scroll-2026-04-27.md` |
...
| Huge transcript viewport changes avoid full-buffer render behavior | `live-terminal measured / scaling risk` | Headless viewport replay: 1,000 entries ~2.15 ms, 10,000 entries ~16.6 ms central / p95 ~17.7 ms; live terminal 10,000-entry scroll p95 4.435 ms over 200 samples; live terminal 50,000-entry scroll p95 22.962 ms over 50 samples. Current behavior appears to scale with transcript size and 50,000-entry stress exceeds a 16 ms frame budget. | Add same-machine Pi large-transcript comparison; plan transcript viewport/render optimization separately if 50,000-entry sessions are in near-term dogfood scope. |
