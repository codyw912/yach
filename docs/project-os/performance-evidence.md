...
| Huge transcript viewport changes avoid full-buffer render behavior | `live-terminal measured / optimized partial` | Baseline live terminal 50,000-entry scroll p95 22.962 ms exceeded a 16 ms frame budget. After transcript render caching, live terminal 50,000-entry scroll p95 is 151.667 µs over 50 samples; 10,000-entry p95 is 152.041 µs over 200 samples. Pi synthetic fixture loading is viable, but Pi scroll latency timing is unsupported until redraw detection is robust. | Pick a comparable Pi-managed timing surface or add deeper Pi-side instrumentation before same-machine large-transcript comparison; measure active streaming with frequent cache invalidation if real dogfood sessions show stream-render pressure. |
