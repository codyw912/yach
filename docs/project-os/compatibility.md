...
| Entry-id fork and fork messages | §6.2 / §6.4 | Tier A stock RPC | RPC | `partial` | `../status/compatibility-evidence-2026-04-27.md`; `just test` 2026-04-27 covers protocol serialization for `fork` with `entryId`/`position` and parsing `get_fork_messages` | Pi RPC exposes `fork` and `get_fork_messages`; yach now models the entry-id fork request and fork-message response, but not session tree data or entry selection UI | Add TUI entry picker/tree on top of structured fork messages. |
