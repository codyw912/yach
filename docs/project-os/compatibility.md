...
| Entry-id fork and fork messages | §6.2 / §6.4 | Tier A stock RPC | RPC | `partial` | `../status/compatibility-evidence-2026-04-27.md`; `just test` 2026-04-27 covers protocol serialization for `fork` with `entryId`/`position`, parsing `get_fork_messages`, and TUI fork-point selection event flow | Pi RPC exposes `fork` and `get_fork_messages`; yach now models the entry-id fork request/response and has a basic TUI fork-point picker, but not a full session tree | Add real session tree/session-file navigation and live smoke entry-id fork against Pi RPC. |
