...
| Existing Pi session files/tree | §6.2 | Session compatibility | RPC / SDK sidecar / native file I/O TBD | `partial` | `../status/m2-tui-checkpoint.md`, `../status/compatibility-evidence-2026-04-27.md`; `just test` 2026-04-27 covers recent-session request/event flow | `sessionFile` is parsed, current branch/entry-id fork works, and yach can scan Pi's per-cwd session directory for recent session files; picker labels are still raw paths, live resume smoke is pending, and full tree browsing/branch summaries/compaction details are missing | Improve recent picker labels, add live resume smoke, then design full tree model. |
