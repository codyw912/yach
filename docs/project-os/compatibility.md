...
## Compatibility strategy note

As of decision `D20260427-01`, the stock Pi RPC adapter is a compatibility/reference layer rather than an exhaustive feature-complete target. Keep tracking evidence for migration-critical and dogfood-critical surfaces, but do not promote small Pi-backend-only parity gaps ahead of native Rust backend planning unless they unblock that path.
...
| Existing Pi session files/tree | §6.2 | Session compatibility | RPC / SDK sidecar / native file I/O TBD | `partial` | `../status/m2-tui-checkpoint.md`, `../status/compatibility-evidence-2026-04-27.md`; `just test` 2026-04-27 covers recent-session request/event flow; `just test` 2026-04-27 on `feat/m3-session-tree-presentation` covers session-message-to-tree branch summaries and Ctrl+B summary presentation | `sessionFile` is parsed, current branch/entry-id fork works, yach can scan Pi's per-cwd session directory for recent session files with human-readable labels, live `switch_session` by `sessionPath` smokes successfully, and Ctrl+B can load a branch summary from typed session messages; richer browsable tree UI and compaction semantics should be owned by native backend design instead of inferred from Pi messages | Use Pi evidence as reference input for native session model planning; avoid chasing Pi-backend-only tree/compaction parity. |
