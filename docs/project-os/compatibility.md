...
| Entry-id fork and fork messages | §6.2 / §6.4 | Tier A stock RPC | RPC | `planned` | `../status/compatibility-evidence-2026-04-27.md` | Pi RPC exposes `fork` and `get_fork_messages`; yach does not model entry ids/tree yet | Add protocol/session tree model before UI. |
| Session stats/messages/export | §6.2 / §6.4 | Tier A stock RPC | RPC | `implemented-unverified` raw smoke for stats/messages; `planned` for structured proto/UI/export | `../status/compatibility-evidence-2026-04-27.md` | `get_session_stats` and `get_messages` raw commands smoke successfully, but responses are not structured in yach-proto/TUI; `export_html` not modeled | Add typed proto/adapter parse coverage for M3. |
