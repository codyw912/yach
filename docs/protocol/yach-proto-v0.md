...
`crates/yach-cli` has explicit non-default native dogfood paths that reuse existing protocol events for the first backend-owned runners: `yach tui --backend native` for the fake/fixture runner and `yach tui --backend native-provider` for an approval-gated Rig provider dogfood path. Both use `Ready`, `StateUpdated`, `AvailableModelsUpdated`, `PromptDelta`, `PromptFinished`, `StatusUpdated`, `SessionMessagesUpdated`, `SessionStatsUpdated`, and `RecentSessionsUpdated`; native-provider additionally routes prompts through `ProviderRequest -> run_provider_request(...) -> ProviderStreamEvent` below `yach-backend` when explicit provider env is configured. Pi remains the default TUI backend.
