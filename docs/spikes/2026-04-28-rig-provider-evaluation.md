...
A subsequent explicit non-default TUI boundary, `yach tui --backend native-provider`, routes native prompt submissions through the same provider-request seam when provider env is configured. Human dogfood confirmed it launched and completed a chat/response turn successfully. The resulting `.yach/native-sessions/default.jsonl` persisted a user entry, assistant entry with provider metadata (`chatgpt-subscription`, `gpt-5.3-codex-spark`, `response_id=null`), and completed turn. Initial state/model list/status now advertise the selected native provider/model for `--backend native-provider` instead of fixture echo. Pi remains the default backend; fixture native mode remains `--backend native`; no provider tools/resources, retry loop, raw payload persistence, or default-backend change were added.
