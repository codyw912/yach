...
A subsequent explicit non-default TUI boundary, `yach tui --backend native-provider`, routes native prompt submissions through the same provider-request seam when provider env is configured. Pi remains the default backend; fixture native mode remains `--backend native`; no provider tools/resources, retry loop, raw payload persistence, or default-backend change were added.
