...
The classifier now treats these `not_found` / `not supported` model-shaped errors as unavailable-model failures, and the CLI failure summary includes the normalized provider error kind. Follow-up native-provider UX polish keeps the protocol unchanged and makes existing status/setup copy more actionable: setup failures are prefixed as native-provider setup failures, runtime failures include snake_case normalized provider kind plus a short hint, and ordinary TUI status avoids raw provider debug detail. No credentials or raw payloads are persisted by yach; the manually pasted evidence contained no secrets.
