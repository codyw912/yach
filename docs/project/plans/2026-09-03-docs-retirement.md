...
**Spec:** `docs/project/specs/2026-09-03-docs-retirement-design.md`
**Source:** plane:YACH-7
...
`README.md` (added in Task 3), `benchmarks/`, `protocol/`, and
`project/{roadmap.md,specs/,plans/,records/}`.
...
in the public entry points (`AGENTS.md`, `README.md`, `docs/README.md`,
`docs/project/roadmap.md`); the permitted exceptions are the `.gitignore`
path entry and the `**Outcome:**` / `**Source:**` headers of specs and plans,
which follow the tracking-provider contract by owner decision. No Markdown
link in a surviving product doc targets a deleted path, and no
current-guidance reference to a deleted path remains in product docs,
`justfile`, `evals/`, `.github/`, or non-test code; the single retired-paths
note in `docs/README.md` is the deliberate exception.
