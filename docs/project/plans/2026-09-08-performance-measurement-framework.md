...
- Produces: `yach_trace::{TraceSink, TraceRecord, TraceScope, parse_records}`.
  ...
  - `TraceSink::mark(&self, scope: TraceScope, label: &str)` and `mark_n(&self, scope, label, n: u32)`.
  - `TraceSink::flush(&self)`; `Drop` flushes.
  - `TraceScope::Startup` | `TraceScope::Turn(String)`.
...
`crates/yach-trace/src/lib.rs` — start with the test module; the API is defined in Step 4.

```rust
#[cfg(test)]
mod tests {
    use super::{TraceParseError, TraceRecord, TraceScope, TraceSink, parse_records};
    ...
    fn marks_append_jsonl_records_in_order() {
        ...
        sink.mark_n(TraceScope::Turn(String::from("turn-1")), "tool_dispatched", 2);
        sink.flush();
    ...
    fn drop_flushes() {
        let path = temp_path();
        {
            let Ok(sink) = TraceSink::open(&path) else { return };
    ...
    fn unknown_keys_are_ignored_and_truncated_line_is_error() {
        let ok = parse_records("{\"t_us\":1,\"scope\":\"startup\",\"label\":\"x\",\"future\":true}\n");
        assert_eq!(ok.map(|r| r.len()), Ok(1));
        let bad = parse_records("{\"t_us\":1,\"scope\":\"startup\",\"label\":\"x\"}\n{\"t_us\":2,\"sco");
        assert_eq!(bad, Err(TraceParseError::TruncatedLine { line_no: 2 }));
    }

    #[test]
    fn write_failure_disables_sink_without_panicking() {
        let path = temp_path();
        let Ok(sink) = TraceSink::open(&path) else { return };
        let _ = std::fs::remove_file(&path);
        // Replace the file with a directory so the next flush fails.
        ...
        let _ = TraceRecord { t_us: 0, scope: String::new(), turn_id: None, label: String::new(), n: None };
    }
}
```
...
Run: `just dev cargo test -p yach-trace`
Expected: compile error, `TraceSink` not found.

- [ ] **Step 4: Implement the crate**

Above the test module in `crates/yach-trace/src/lib.rs`:
...
impl fmt::Display for TraceOpenError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "cannot open trace file {}: {}", self.path.display(), self.source)
    }
}
...
impl fmt::Debug for TraceSink {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TraceSink").field("path", &self.path).finish_non_exhaustive()
    }
}
...
impl TraceSink {
    ...
    pub fn open(path: &Path) -> Result<Self, TraceOpenError> {
        let file = OpenOptions::new()
            ...
            .append(true)
            .open(path)
            .map_err(|source| TraceOpenError { path: path.to_path_buf(), source })?;
        Ok(Self {
            start: Instant::now(),
            path: Arc::new(path.to_path_buf()),
            inner: Arc::new(Mutex::new(Inner { writer: Some(BufWriter::new(file)), warned: false })),
        })
    }

    pub fn mark(&self, scope: TraceScope, label: &str) {
        self.write(scope, label, None);
    }

    pub fn mark_n(&self, scope: TraceScope, label: &str, n: u32) {
        self.write(scope, label, Some(n));
    }

    fn write(&self, scope: TraceScope, label: &str, n: Option<u32>) {
        let t_us = u64::try_from(self.start.elapsed().as_micros()).unwrap_or(u64::MAX);
        let (scope, turn_id) = match scope {
            TraceScope::Startup => (String::from("startup"), None),
            TraceScope::Turn(id) => (String::from("turn"), Some(id)),
        };
        let record = TraceRecord { t_us, scope, turn_id, label: label.to_owned(), n };
    ...
    pub fn is_disabled(&self) -> bool {
        self.inner.lock().map(|inner| inner.writer.is_none()).unwrap_or(true)
    }

    fn disable(&self, inner: &mut Inner, error: &std::io::Error) {
        inner.writer = None;
        if !inner.warned {
            inner.warned = true;
            // The one permitted stderr line: the sink is off for the rest of the process.
            let _ = writeln!(
                std::io::stderr(),
...
pub fn parse_records(contents: &str) -> Result<Vec<TraceRecord>, TraceParseError> {
    ...
    while let Some((index, line)) = lines.next() {
        ...
        let record: TraceRecord = serde_json::from_str(trimmed)
            .map_err(|error| TraceParseError::Malformed { line_no, message: error.to_string() })?;
        records.push(record);
    }
    Ok(records)
}
...
`writeln!(std::io::stderr(), …)` is allowed under the workspace lints: `clippy::print_stderr` fires only on the `eprint!`/`eprintln!` macros.
...
```bash
jj commit -m "Add yach-trace append-only JSONL trace sink"
```
...
fn mark_turn(trace: Option<&yach_trace::TraceSink>, turn_id: &TurnId, label: &str) {
    if let Some(trace) = trace {
        trace.mark(yach_trace::TraceScope::Turn(turn_id.0.clone()), label);
    }
}

fn mark_turn_n(trace: Option<&yach_trace::TraceSink>, turn_id: &TurnId, label: &str, n: u32) {
    if let Some(trace) = trace {
        trace.mark_n(yach_trace::TraceScope::Turn(turn_id.0.clone()), label, n);
    }
}
```
