...
- Approval mode foundations are live. `review` remains the conservative
  ...
  transactions while bash retains allowlist/ask behavior. Modes negotiate over
  protocol, persist in project-keyed user state, produce durable session
  evidence, and remain visible in `/status` and the status bar. Repository
  config cannot grant shell/environment authority, and permission config is a
  protected edit path.
