...
- Native edit transactions now have merged backend primitives for preview and guarded apply. `NativeEditEngine::preview` validates create/modify requests, mints yach-owned edit IDs, enforces project-root and metadata path policy, rejects symlink paths, checks expected hashes, applies exact hunks in memory, rejects duplicate targets, and returns bounded diff summaries without writing files. `NativeEditEngine::apply` remains crate-internal, consumes a prepared transaction, hard-rejects multi-operation apply, performs guarded create/modify writes, and returns structured apply metadata. The accepted design remains the basis for edit session evidence, local harnessing, benchmarks, and later provider/tool exposure.
...
The accepted native edit transactions design plus the merged preview/apply engine are sufficient as the basis for a focused edit evidence and local harness plan: explicit redacted session events, JSONL round-trip coverage, and a backend-local wrapper around preview/apply that is not provider-advertised and does not register mutation tools. They are not sufficient for provider-advertised edit tools, extension-owned mutation tools, approval UI, CLI/TUI edit commands, benchmarks, delete/rename, shell/process tools, network tools, verification actions, or multi-operation atomicity; those need focused follow-up specs/plans.
