...
The accepted native edit transactions design plus the merged preview engine are sufficient as the basis for the next implementation plan: backend-owned guarded apply for prepared create/modify transactions, one operation by default, create no-overwrite publish, modify final hash guards, same-directory temporary files, ordinary permission preservation, no-write-on-validation-failure tests, and structured apply metadata. They are not sufficient for provider-advertised edit tools, extension-owned mutation tools, approval UI, edit session evidence, hidden/local edit tool harnessing, benchmarks, delete/rename, shell/process tools, network tools, verification actions, or multi-operation atomicity; those need focused follow-up specs/plans.
