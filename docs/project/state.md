...
- Native edit transactions now have an accepted transaction-engine-first design for a yach-owned preview/apply boundary, exact-match text hunks, create-file transactions, project-root path policy, hash/race guards, no-overwrite creates, redacted edit evidence, and edit benchmarks. Implementation has not started; provider-visible edit tools remain out of scope until a later approval/tool-exposure design.
...
The accepted native edit transactions design is sufficient as the basis for the next implementation plan: a backend-owned `NativeEditEngine` with preview/apply separation, structured exact-match hunks, create-file transactions, project-root path policy, hash/race guards, redacted edit evidence, and edit benchmarks. It is not sufficient for provider-advertised edit tools, extension-owned mutation tools, approval UI, delete/rename, shell/process tools, network tools, verification actions, or multi-operation atomicity; those need focused follow-up specs/plans.
