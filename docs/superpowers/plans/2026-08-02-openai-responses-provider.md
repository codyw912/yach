...
**Architecture:** One new enum variant, one new match arm delegating to the existing `PreparedCompletion::run`, env parsing in the CLI's provider match sites, and a fourth smoke function through the shared smoke helpers. Spec: `docs/superpowers/specs/2026-08-02-openai-responses-provider-design.md`.
