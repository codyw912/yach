...
**Architecture:** `docs/project/` becomes the active project-orientation layer with three live docs plus records. Stock Superpowers specs/plans remain task-level workflow artifacts under `docs/superpowers/`; the live project docs link only the currently relevant artifacts.
